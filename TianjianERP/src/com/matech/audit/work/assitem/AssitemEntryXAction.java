...
public class AssitemEntryXAction  extends MultiActionController{
	private final String _strSuccess = "/assitementryX/List.jsp";
	private final String _strBList = "/assitementryX/BList.jsp";
	...
	public ModelAndView get(HttpServletRequest request, HttpServletResponse response)  throws Exception {
		...
		try {
			...
			out.print(assitem.getAssXProject(acc, assitem.getAssItemName(acc, AssItemID),assitem.getAssItemName(acc, SubjectID) , AssItem));
			out.close();
		} catch (Exception e) {
			Debug.print(Debug.iError, "核算项目明细账查询失败！", e);
            throw e;
...
//			String EndYear = CHF.showNull(request.getParameter("EndYear"));
			...
			String a_accid = CHF.showNull(request.getParameter("a_accid"));
			String a_subjectids = CHF.showNull(request.getParameter("a_subjectids"));
...
//			System.out.println("py0 AssItem:=" + (String)request.getAttribute("AssItem"));
			...
			*/
			...
			switch(result){
			case 0://bonelist				
				request.getRequestDispatcher("assitementryX.do?method=bonelist&AccPackageID="+acc+"&TabName="+TabName+"&AssItemID="+AssItemID+"&SubjectID="+SubjectID+"&AssItem="+AssItem+"&currency="+currency+"&a_accid="+a_accid+"&a_subjectids="+a_subjectids).forward(request, response);
				break;
			case 1://btwolist
			
				request.getRequestDispatcher("assitementryX.do?method=btwolist&AccPackageID="+acc+"&TabName="+TabName+"&AssItemID="+AssItemID+"&SubjectID="+SubjectID+"&AssItem="+AssItem).forward(request, response);
				break;
			case 2://bthreelist
				request.getRequestDispatcher("assitementryX.do?method=bthreelist&AccPackageID="+acc+"&TabName="+TabName+"&AssItemID="+AssItemID+"&SubjectID="+SubjectID+"&AssItem="+AssItem).forward(request, response);
				break;
			case 3://bfourlist
				request.getRequestDispatcher("assitementryX.do?method=bfourlist&AccPackageID="+acc+"&TabName="+TabName+"&AssItemID="+AssItemID+"&SubjectID="+SubjectID+"&AssItem="+AssItem).forward(request, response);
				break;
			}
	...
	public ModelAndView bonelist(HttpServletRequest request, HttpServletResponse response)  throws Exception {
		...
		try {
			...
			String a_accid = CHF.showNull(request.getParameter("a_accid"));
			String a_subjectids = CHF.showNull(request.getParameter("a_subjectids"));
			String bAssItemName = CHF.showNull(request.getParameter("bAssItemName"));
			String cAssItemName = CHF.showNull(request.getParameter("cAssItemName"));
			...
			if(!"".equals(AssItem)){
				String countname1 = assitem.getAssTotalName(acc,AssItemID);
				String countname2 = assitem.getAssTotalName(acc,SubjectID);
				String sTable = "<table width=\"100%\" cellspacing=\"0\" cellpadding=\"0\" border=\"0\"><tr><td>"+countname1+"：<font color='blue'>"+bAssItemName+"</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+countname2+"：<font color='blue'>"+cAssItemName+"</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;年度：<font color='blue'>"+acc.substring(6)+"</font></td></tr></table>";
				...
				if(!assitem.ExistsTable(TabName)){
					...
					assitem.DataToTableX(TabName, user, projectid, acc.substring(0,6), SubjectID, AssItemID, acc.substring(6), BeginDate, acc.substring(6), EndDate,a_accid,a_subjectids);
				...
				if(!"".equals(AssItemID) && !"".equals(SubjectID)){
					...
					String sql = "select id,a.vchmonth vchmonth1,a.voucherid,autoid," +
							...
							"from (select *,group_concat(distinct "+subjectnameSql+") subjectnames   from (select * from `"+TabName+"` order by id ${LIMIT}) a left join (select * from c_accpkgsubject where  AccPackageID="+acc+" "+sqlwhere+" )   b  on 1=1 and a.subjects like concat('%,',b.subjectid,',%')  group by a.id  ) a " +
					...
					pp.setTrAction("  AccPackageID='"+acc+"' AssitemID='${assitemid}' VchDate='${vchdate}' TypeId='${typeid}' OldVoucherId='${oldvoucherid}' autoid='${autoid}' style='cursor:hand;' onDBLclick='goSort();'");
		...
		try {
			...
			if(!"".equals(AssItem)){
				...
				if(!assitem.ExistsTable(TabName)){
					...
					assitem.DataToTable(map,TabName, user, projectid, acc.substring(0,6), SubjectID, AssItemID, acc.substring(6), BeginDate, acc.substring(6), EndDate,1);
				}
				...
				if(!"".equals(AssItemID) && !"".equals(SubjectID)){
					...
					TableHead = "月,凭证日期,字,号,核算编号,${subjectid},摘要,";
					...
					if(acc.equals(userSession.getCurAccPackageId()) && userSession.getCurProjectId()!=null){
						TableHead = "抽,疑,月,凭证日期,字,号,核算编号,${subjectid},摘要,";
						pp.addColumn("抽","p1",FormatType.showTakeOut );
						pp.addColumn("疑","p2",FormatType.showDoubt );	
						ProjectService projectService = new ProjectService(conn);
						if("税务审计".equals(projectService.getProjectById(projectid).getAuditPara())){
	...
	public ModelAndView bthreelist(HttpServletRequest request, HttpServletResponse response)  throws Exception {
		...
		try {
			...
			if(!"".equals(AssItem)){
				...
				if(!assitem.ExistsTable(TabName)){
					assitem.CreateTable(TabName,map);				
					assitem.DataToTable(map,TabName, user, projectid, acc.substring(0,6), SubjectID, AssItemID, acc.substring(6), BeginDate, acc.substring(6), EndDate,2);
				}
		...
		try {
			...
			if(!"".equals(AssItem)){
				...
				if(!assitem.ExistsTable(TabName)){
					...
					assitem.DataToTable(Cmap,Umap,TabName, user, projectid, acc.substring(0,6), SubjectID, AssItemID, acc.substring(6), BeginDate, acc.substring(6), EndDate);				
				}
				...
				if(!"".equals(SubjectID)){
					...
					pp.setTrActionProperty(true);
					pp.setTrAction("  AccPackageID='"+acc+"' VchDate='${vchdate}' TypeId='${typeid}' OldVoucherId='${oldvoucherid}' style='cursor:hand;' onDBLclick='goSort();'");
...
//			printSetup.setIColumnWidths(new int[]{4,13,18,5,24,72,22,20,4,23});
		//	printSetup.setVertical(true);	 
			...
			String countname1 =  new Assitem(conn).getAssTotalName(acc,AssItemID);
			String countname2 =  new Assitem(conn).getAssTotalName(acc,SubjectID);
			
			List[] lists = new List[1];
			lists[0] = new ArrayList();  
			lists[0].add(0, "1");
			lists[0].add(1, countname1+"："+bAssItemName+"  "+countname2+"："+cAssItemName+"  年度："+acc.substring(6)+"");
			lists[0].add(2, "9");
			lists[0].add(3, null);
