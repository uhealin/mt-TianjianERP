 ...
 *
 */
public class _1015_0 extends AbstractAreaFunction {
	public ResultSet process(HttpSession session, HttpServletRequest request,
		...
		try {
			...
			String showtype = CHF.showNull(request.getParameter("showtype"));
			if("".equals(showtype)){
				showtype = CHF.showNull((String) args.get("对方科目")); 
			}
			//刷分录还是整张凭证 showtype=1 刷抽凭科目=凭证科目的分录；showtype=0 刷抽凭科目<>凭证科目的分录
...
//							"and submonth=1   \n" +
			...
			if(assitem == null || "".equals(assitem)){
				...
				sql = " "+sql+" where 1=1 "+sqlJudge+" order by  Createor,vchdate,typenumber,Serail,AssItemName"; 
			}
	...
	public String getSql1(int iMode,String direction,String opt,String dir,String showtype){
		...
		if("1".equals(showtype)) {
			...
			st = " AND not ((a.subjectid LIKE CONCAT(a.entrySubjectID,'%')) OR  (a.entrySubjectID LIKE CONCAT(a.subjectid,'%')))" ;
		}
		...
		if (iMode == 1) {//刷单边
			...
			"		group_concat(distinct b.name) Createor,group_concat(distinct IF(IFNULL(judge,'') = '',NULL,judge)) judge,  \n" +
			"		ifnull(f.assitemid,'') as assitemid,ifnull(f.assitemname,entrysubjectfullname1) as assitemname, " +
			"	entrytypeid,entryoldvoucherid " +
			"		from z_voucherspotcheck a  \n" +
...
//			str4 = " and subjectid in (${subjectid})  and accid in (${subjectid}) ";
		...
		if (iMode == 1) {
			...
			strSql ="\n select * from ( \n" + 
					...
					"	if(left(other,1)=',',substring(other,2),other) as othersubjectname,  \n" +
					"	if(left(other,1)=',',substring(other,2),other) as subjectname2,  \n" +
					"	'' as assitemid,'' as assitemname,entrytypeid,entryoldvoucherid " +
					"	from(  \n" +
					"		select a.*, a.subjectname1 as   subjectname, replace(b.allsubjectname,a.accname,'')  as other  \n" +
					"		from ( \n" +
					...
					"			case entrydirction when 1 then concat('借:',entrysubjectfullname1) else concat('贷:',entrysubjectfullname1) end as subjectname22,  \n" +
					"			b.Createor,judge,entryAccPackageID,entrysubjectid,entryoldvoucherid,entryVchDate,entrytypeid,entrySerail,entrybankid \n" +
					"			from z_voucherspotcheck a, \n" +
...
//					"	left join c_assitementry b \n" +
//					"	on a.voucherid = b.voucherid  and entrySerail = b.Serail and a.entrysubjectid=b.subjectid \n" +
//					"	left join ( \n" +
//					" 		select distinct AccPackageID,assitemid,assitemname  \n" +
//					"		from c_assitem    \n" +
//					" 	)c on a.entryAccPackageID = c.AccPackageID and b.assitemid = c.assitemid \n" +
//					
//					"	group by vchdate,voucherid,Serail, typenumber,summary, accname,other,othersubjectname,subjectname2,debitocc,creditocc,occ,Createor,judge " +
					" )a ";
			...
			//刷全部
			strSql = "\nselect * from ( \n" +
					"\n	select a.vchdate,a.voucherid,a.Serail, typenumber,a.summary, subjectname1," +
					"	concat(subjectname,entrybankid) as subjectname, \n" +
					"	concat(othersubjectname,entrybankid) as othersubjectname,subjectfullname22,\n" +
					"	concat(subjectname2,entrybankid) as subjectname2,debitocc,creditocc,occ,Createor, judge, \n" +
					"	'' as assitemid,'' as assitemname,entrytypeid,entryoldvoucherid " +
					"	from ( \n" +
...
//					"	on a.voucherid = b.voucherid  and entrySerail = b.Serail and a.entrysubjectid=b.subjectid \n" +
//					"	left join ( \n" +
//					" 		select distinct AccPackageID,assitemid,assitemname from c_assitem    \n" +
//					"	)c on a.entryAccPackageID = c.AccPackageID and b.assitemid = c.assitemid \n" +
//					
//					"	group by vchdate,voucherid,Serail, typenumber,summary, subjectname1,subjectfullname22,debitocc,creditocc,occ,Createor,judge " +
