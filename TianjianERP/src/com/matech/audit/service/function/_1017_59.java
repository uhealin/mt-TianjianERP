...
public class _1017_59 extends AbstractAreaFunction {

	/**
	 * 就是1007公式，但是增加了：
	 * 返回按审定余额降序排序的前N条；N可以由前台提供
	 * 返回前N条参数：limit，如果没有设置，就按照默认5条返回；
	 * 前台设置的方法为：
	 * =取列公式覆盖(1017, "", "subjectname","&subjectname=应收账款&starmonth=12&endmonth=12&ptype=1",3)
	 */
	public ResultSet process(HttpSession session, HttpServletRequest request, HttpServletResponse response, Connection conn, Map args) throws Exception {
		...
		//获取前台提交的限制值，如果没有提供就按照默认5条进行限制
		String limit=(String)request.getParameter("limit");
		if (limit==null || limit.equals("") ||  limit.equals("9999") ){
			limit="5";
		}
...
//			判断该科目是否叶子并且有自增科目。
			...
			System.out.println("qwh:1017:resultsql="+resultSql);
	...
	public String getSql(String rectifySign,String subjectid){
		...
		+"        IF(b2.assitemid is null,a2.sov3,b2.sov3) * a2.rectifySign as fbalance, \n"
		...
		+"        IF(b2.assitemid is null,a2.debitsov2+a2.debitsov3+a2.debittotalocc,b2.debitsov2+b2.debitsov3+b2.debittotalocc) * a2.rectifySign as sddebittotalocc,  \n"
		+"        IF(b2.assitemid is null,a2.creditsov2+a2.creditsov3+a2.credittotalocc,b2.creditsov2+b2.creditsov3+b2.credittotalocc) * a2.rectifySign as sdcredittotalocc, \n"
		...
		+" order by sdbalance desc limit ${limit}";
	}
}
