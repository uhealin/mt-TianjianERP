...
public class _1039_1 extends AbstractAreaFunction {
	...
	public ResultSet process(HttpSession session, HttpServletRequest request, HttpServletResponse response, Connection conn, Map args) throws Exception {
		...
		try{
			...
			if (month.equals("xxx")){
				//取调整数
				sql="select a.*,if(b.occurvalue is null,0,b.occurvalue)* ("+ptype+") as occ \n"
					+"from \n"
					+"( \n"
				...
				if (bHasEntry){
				//取余额表发生数（本年的或者上年/下年的）
				sql="select a.*, (if(b.occ1 is null,0,b.occ1)-if(c.occ2 is null,0,c.occ2)+ if(d.occurvalue is null,0,d.occurvalue))* ("+ptype+") as occ \n"
					+"from \n"
					+"( \n"
					+"	select distinct accname as subjectname,subjectfullname1 as subjectfullname \n"
					+"	from c_account  \n"
					...
					+"left join \n" 
					+"( \n"
					+"	select subjectfullname1 ,debittotalocc-credittotalocc as occ1 \n"
					+"	from c_account  \n"
					...
					+"	and   b.subjectid in ('"+myds+"') \n" 
					+")d \n"
					+"on a.subjectfullname=d.subjectfullname \n"
...
//					没有记录的情况下，先考虑借发生－贷发生，为0的话就取同方向的发生
					...
					sql="select a.*, (case occ1 when 0 then debittotalocc else occ1 * direction2 end )+ if(d.occurvalue is null,0,d.occurvalue) * direction2 as occ \n"
					+"from \n"
					+"( \n"
					+"	select distinct accname as subjectname,subjectfullname1 as subjectfullname \n"
					+"	from c_account  \n"
