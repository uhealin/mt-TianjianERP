...
public class _1044_0 extends AbstractAreaFunction {
	 ...
	 * 最终返回值就是 A1-A2
	 * 
	 * 参照1039的公式算法，比较ds和os的值
	 * 公式的设置方法：
	 * =取列公式覆盖(1044, "", "occ", "&s=;&cs=本年利润;利润分配&ds=其他业务收入&os=其他业务支出&dt=1&year=-1&thisyear=1")
	 * =取列公式覆盖(1044, "", "occ", "&s=;&cs=本年利润;利润分配&ds=其他业务支出&os=其他业务收入&dt=-1&year=-1&thisyear=1") 
	 * 其实就是ds和os交换
	 * 
	 * dt始终是方向
	 * 
	...
	public ResultSet process2(HttpSession session, HttpServletRequest request, HttpServletResponse response, Connection conn, Map args) throws Exception {
		...
		try {
			...
			//第三个科目，用于显示科目(排排坐
			String os=(String)args.get("os");
			os=asf.replaceStr(os, s, "','");
			String myos="";
...
//			最后的组装
			if (month.equals("xxx")){
				//取调整数
				sql="select replace (replace (subjectname ,'收入',''),'成本','') as subjectname ,sum(occ) as occ \n"
					+"from ( \n"
					+"select if(a.level1>=2,replace(a.subjectfullname,substr(a.subjectfullname,1,if(locate('/',a.subjectfullname) = 0,length(a.subjectfullname),locate('/',a.subjectfullname))),''),a.subjectfullname) as subjectname,a.subjectfullname,if(b.occurvalue is null,0,b.occurvalue)* ("+ptype+") as occ \n"
					+"from \n"
					+"( \n"
					...
					+"	or (subjectfullname2 = '"+os+"' or subjectfullname2 like '"+os+"/%'))\n"
					+")a  \n"
					+"left join \n" 
					+"( \n"
					...
					+") a \n" 
					+"group by replace (replace (subjectname ,'收入',''),'成本','') \n"
					+"order by replace (replace (subjectname ,'收入',''),'成本','') ";
			}else{
				//取余额表发生数（本年的或者上年/下年的）
				if(bHasEntry){
					sql="select replace (replace (subjectname ,'收入',''),'成本','') as subjectname ,sum(occ) as occ \n"
					+"from ( \n"
					+"select if(a.level1>=2,replace(a.subjectfullname,substr(a.subjectfullname,1,if(locate('/',a.subjectfullname) = 0,length(a.subjectfullname),locate('/',a.subjectfullname))),''),a.subjectfullname) as subjectname,a.subjectfullname, (if(b.occ1 is null,0,b.occ1)-if(c.occ2 is null,0,c.occ2))* ("+ptype+") as occ \n"
					+"from \n"
					+"( \n"
					+"	select distinct accname as subjectname,subjectfullname1 as subjectfullname,level1 \n"
					+"	from c_account  \n"
					...
					sql="select replace (replace (subjectname ,'收入',''),'成本','') as subjectname ,sum(occ) as occ \n"
					+"from ( \n"
					+"select if(a.level1>=2,replace(a.subjectfullname,substr(a.subjectfullname,1,if(locate('/',a.subjectfullname) = 0,length(a.subjectfullname),locate('/',a.subjectfullname))),''),a.subjectfullname) as subjectname,a.subjectfullname,  "
					+"case occ1 when 0 then if(direction2=1,debittotalocc,credittotalocc ) else occ1 * direction2 end as occ \n"
					+"from \n"
					+"( \n"
					+"	select distinct accname as subjectname,subjectfullname1 as subjectfullname,level1 \n"
					+"	from c_account  \n"
					...
					+"left join \n" 
					+"( \n"
					+"	select subjectfullname1 ,debittotalocc-credittotalocc as occ1,debittotalocc,credittotalocc,direction2 \n"
					+"	from c_account  \n"
					+"	where 1=1 "
					+"	and SubYearMonth*12+SubMonth>="+strBegin+" \n"
					+"	and SubYearMonth*12+SubMonth<="+strEnd+" \n"
...
//				+" and accpackageid ="+nowAccpackageid+" \n" 
			...
			*/
			...
			if (month.equals("xxx")){
				//取调整数
				sql="select subjectname ,sum(occ) as occ \n"
					+"from ( \n"
					+"select if(a.level1>=2,replace(a.subjectfullname,substr(a.subjectfullname,1,if(locate('/',a.subjectfullname) = 0,length(a.subjectfullname),locate('/',a.subjectfullname))),''),a.subjectfullname) as subjectname,a.subjectfullname,if(b.occurvalue is null,0,b.occurvalue)* ("+ptype+") as occ \n"
					+"from \n"
					+"( \n"
					...
					+"	where accpackageid in("+accpackageid+","+anotherApkID+") \n"
					+"	and isleaf1=1 \n"
					+"	and (subjectfullname2 like '"+ds+"%' or subjectfullname2 like '"+os+"%')\n"
					+")a  \n"
					+"left join \n" 
					+"( \n"
					+"	select b.subjectfullname,(DebitTotalOcc1+DebitTotalOcc2-CreditTotalOcc1-CreditTotalOcc2) as occurvalue \n"
