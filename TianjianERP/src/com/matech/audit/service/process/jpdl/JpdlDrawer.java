 ...
 * 
 */
public class JpdlDrawer {
	public static final int RECT_OFFSET_X = JpdlProperty.RECT_OFFSET_X;
	public static final int RECT_OFFSET_Y = JpdlProperty.RECT_OFFSET_Y;
	...
	private final static Map<String, Object> nodeInfos = JpdlProperty
			.getNodeInfos();

	public BufferedImage draw(JpdlProperty jpdlProperty) throws IOException {
		Rectangle dimension = getCanvasDimension(jpdlProperty);
		...
		g2.setFont(font);
		Map<String, Node> nodes = jpdlProperty.getNodes();
	 ...
	 * @return
	 */
	private Rectangle getCanvasDimension(JpdlProperty jpdlProperty) {
		Rectangle rectangle = new Rectangle();
		Rectangle rect;
		for (Node node : jpdlProperty.getNodes().values()) {
	...
	private void drawTransition(Map<String, Node> nodes, Graphics2D g2)
		...
		for (Node node : nodes.values()) {
			for (Transition transition : node.getTransitions()) {
				...
				Point p = JpdlUtil.getRectangleLineCrossPoint(
				...
				p = JpdlUtil.getRectangleLineCrossPoint(toNode
	...
	private void drawNode(Map<String, Node> nodes, Graphics2D g2, Font font)
		...
		for (Node node : nodes.values()) {
			...
			if (nodeInfos.get(node.getType()) != null) {
			//	System.out.println(getClass().getResource("icons/48/" + nodeInfos.get(node.getType())).getPath());
	...
	public static void main(String[] args) throws Exception{
		JpdlProperty jbdlProperty = new JpdlProperty(JpdlProperty.class.getResource("test.jpdl.xml").openStream()) ;
		ImageIO.write(new JpdlDrawer().draw(jbdlProperty), "png", new File("c:/loan.png")); 
	}
