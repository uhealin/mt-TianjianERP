package com.matech.audit.service.oa.target;
...
public class TargetService {
	...
	public Target getTargetAndProByTaskId(String taskId) {
		...
		try {
			...
			String strSql = "select * from jbpm_z_target where taskId = ? and projectid = ?";
	...
	public List getTargetAndProList() {
		...
		try {
			...
			String strSql = "select a.* from jbpm_z_target a,z_task b "
	...
	public List getTargetAndProListByTaskId(String parentTaskId) {
		...
		try {
			...
			String strSql = " select a.* " + " from jbpm_z_target a,jbpm_z_task b "
					+ " where b.property = 'target'  "
	...
	public List getTargetListByTaskId(String taskId) throws Exception {
		...
		try {
			...
			String strSql = " Select * From jbpm_z_target "
					+ " Where ProjectId =  ? "
	...
	public void addTarget(Target target) throws Exception {
		...
		try {
			...
			String strSql = "insert into jbpm_z_target (ProjectID, TaskID, ExecuteIt, State, DefineID, AuditTarget, CorrelationExeProcedure, Remark,cognizance,property) "
					+ " values(?,?,?,?,?,?,?,?,?,?)";
			ps = conn.prepareStatement(strSql);
	...
	public int updateTarget(Target target) throws Exception {
		...
		try {
			...
			String strSql = "update jbpm_z_target set ExecuteIt=? , State=?, DefineID=?, AuditTarget=?, CorrelationExeProcedure=?, Remark=?,cognizance=?,property=? "
	...
	public void updateTarget(String taskId, String att, String val) {
		...
		try {
			...
			String strSql = "update jbpm_z_target set " + att + " = ? "
					+ " where taskId = ? and projectId = ?";
			ps = conn.prepareStatement(strSql);
	...
	public int removeTargetByTaskId(String taskId) throws Exception {
		...
		try {
			...
			String strSql = "delete from jbpm_z_target where taskid = ? and projectid = ?";
			ps = conn.prepareStatement(strSql);
			ps.setString(1, taskId);
			ps.setString(2, projectId);
			return ps.executeUpdate();
	...
	public int removeTargetByAutoId(String autoId) throws Exception {
		...
		try {
			...
			String strSql = "delete from jbpm_z_target where autoId = ? ";
			ps = conn.prepareStatement(strSql);
	...
	public boolean hasChildren(String taskId, String Property) {
		...
		try {
			String strSql = "select 1 from jbpm_z_task where ParentTaskID = ? and projectid = ? and Property = ?";
			ps = conn.prepareStatement(strSql);
			ps.setString(1, taskId);
			ps.setString(2, projectId);
	...
	public boolean hasChildren(String taskId) {
		...
		try {
			...
			String strSql = "select 1 from jbpm_z_task where ParentTaskID = ? and projectid = ? ";
			ps = conn.prepareStatement(strSql);
			ps.setString(1, taskId);
			ps.setString(2, projectId);
			rs = ps.executeQuery();
	...
	public void createTable() throws Exception {
		dropTable();
		String sql = " create table " + this.tempTableName + " like jbpm_z_target ";
		PreparedStatement ps = null;
		try {
	...
	public void insertData2() throws Exception {
		...
		try {
			...
			String sql2 = " insert into jbpm_z_target(DefineID, AuditTarget, cognizance,state, CorrelationExeProcedure, projectId, taskId, autoId ) "
					+ " values(?,?,?,?, ?,?,?,?) ";
	...
	public void insertData() throws Exception {
		PreparedStatement ps = null;
		try {
			String sql = "insert into jbpm_z_target(DefineID, AuditTarget, cognizance,state, CorrelationExeProcedure, projectId, taskId) "
	...
	public String getMaxAutoId() throws Exception {
		...
		String sql = " select ifnull(max(autoId),0) + 1 " + " from jbpm_z_target "
				+ " where projectId=? ";
