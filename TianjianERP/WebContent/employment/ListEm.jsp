    ...
    */
	...
	var tbar_user = new Ext.Toolbar({
		...
		},
       {
			...
			icon:'${pageContext.request.contextPath}/img/query.gif',
			handler:function () {
				var uid=document.getElementById("chooseValue_user").value;
				if(uid==""){
					alert("请选择要查看的对象！");
				}else{
					$.getJSON("${pageContext.request.contextPath}/user.do",{method:"jsonUser",id:uid},function(json){
					    var url="${pageContext.request.contextPath}/oa/UserEmFrame.jsp?myUserid="+uid;
						var n= mytab.add({
					    	id:uid,
					    	closable:true,
					    	title:json["name"],
...
<div id="divBatchUpdate" style="display: none;">
  <table class="formTable">
     <tbody>
       <tr>
          <th>子集</th>
          <td><input autoid="5011" noinput="true" name="item" id="item" /></td>
        </tr>
        <tr>
          <th>属性</th>
...
tdObj.align = "center";
	...
	var columnHtml = "" //"<div class=selectDiv style=\"width:120px;\">"
	    ...
	    +"<input autoid='5012|query_table_" + tableId +r+ "' noinput='true' noinput='true' refer='query_table_" + tableId +r+ "' name='query_column_" + tableId + "' id='query_column_" + tableId +r+ "'>"
	;
	    /*				
    ...
    */
	...
	//+ "	<select class=mySelect style=\"width:80px;\" name='query_operator_" + tableId + "' id='query_operator_" + tableId +r+ "'>"
	...
	//+ " </select>"
	+" <input value='like'  name='query_operator_" + tableId + "' noinput='true' id='query_operator_" + tableId+r + "' autoid='700|query_operator' refer='query_operator' style='width:100px' />" ;
;
	//+ " </div>";
	//����
	tdObj = trObj.insertCell();
...
function addEmQueryWithValue(tableId,first,item) {
	...
	tdObj.innerHTML = "<div  style=\"width:100%;"+display+"\">"
					//+ "<select class=mySelect style=\"width:80px;\" name='query_logic_"+tableId+"' id='query_logic_" + tableId + "' value='"+item["logic"]+"' />"
					//+ "		<option value='and'>与(and)</option>"
					//+ "		<option value='or'>或(or)</option>"
					//+ "</select>"
	...
	var tableHtml = "" //"<div class=selectDiv style=\"width:200px;\">"
		//+ "	<select class=mySelect style=\"width:200px;\" name='query_table_" + tableId + "' id='query_table_" + tableId + "'>" ;
     +"<input autoid='5011' name='query_table_" + tableId + "' noinput='true' id='query_table_" + tableId +r+ "' noinput='true'  value='"+item["table_name"]+"' />"
		//for(var i=0;i<jarrEmTable.length;i++){
    //	tableHtml+="<option value='" + jarrEmTable[i]["table_name"] + "'> " +jarrEmTable[i]["table_name_cn"]  + " </option>" ;
...
tdObj.align = "center";
	...
	var columnHtml = "" //"<div class=selectDiv style=\"width:120px;\">"
					//+ "	<select class=mySelect style=\"width:120px;\" name='query_column_" + tableId + "' id='query_column_" + tableId + "'>" ;
	    +"<input autoid='5012|query_table_" + tableId +r+ "' noinput='true'  refer='query_table_" + tableId +r+ "' noinput='true' name='query_column_" + tableId + "' id='query_column_" + tableId +r+ "' value='"+item["column_name"]+"' />"
	/*				
	for(var i=0;i<columns.length;i++) {
		var id = columns[i].freequery || columns[i].id ;
		var header = columns[i].header ;
