...
function exit(){
	new Ext.Toolbar({
		renderTo:'GridDiv_cpaNianJianList',
		items:[{ 
				text:'查询',
...
<div class="autoHeightDiv">
	<mt:DataGridPrintByBean name="cpaNianJianList" />
</div>
	...
	<form name="thisForm" id="thisForm" method="post">
		...
		<table border="0" cellpadding="5" cellspacing="10" width="100%" align="center">
			...
			<tr>
				...
				<td align=left>
					<input type="text" name="alreadyCreate" id="alreadyCreate" autoid=10001 refer="是否" size="21"/>
				</td>
			</tr>
		</table>		
	</form>		
...
//修改项目生成模式
 function edit(){
	 var chooseValue = document.getElementById("chooseValue_cpaNianJianList").value;
 ...
 function createReport(){
	 ...
	 if(chooseValue==""){
		 ...
		 var url="nianJian.do?method=edit&chooseValue="+chooseValue;
		 parent.openTab(new Date().getTime(),"生成注师年检申报表",url);
	 }
...
function queryDetail(){
	var chooseValue = document.getElementById("chooseValue_cpaNianJianList").value;
	 ...
	 if(chooseValue==""){
		 ...
		 var url="nianJian.do?method=edit&chooseValue="+chooseValue+"&opt=view";
		 parent.openTab(new Date().getTime(),"生成注师年检申报表",url);
	 }
}
</script>
