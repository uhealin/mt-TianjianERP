...
<form name="thisForm" method="post" action="" id="thisForm" > 
	...
	<table border="0"  style="line-height: 28px"   class="data_tb" align="center">
	   ...
	   <tr>
		...
		<tr >
			<td class="data_tb_alignright" align="right">项目编号：</td>
			<td class="data_tb_content" colspan="3"><input type="text" id="projestId"
				maxlength="20" readonly="readonly" size="40" name="projestId" value="${newCustomer.projestId}" />
				<font color="red" size=3>(项目编号由市场部审核时填写)</font>
			</td>
		</tr>
		<tr>
		...
		<c:if test="${ audit !=null}">
			<table class="data_tb" align="center">
				<tr>
						<th class="data_tb_alignright" align="center" colspan="4">我所项目负责人</th>
				</tr>
					<tr>
						<td class="data_tb_alignright"  nowrap="nowrap"> 合伙人<font color="red" size=3>[*]</font>：
						</td>
						<td class="data_tb_content">
						<input type="text" id="projestPartner" 
							onfocus="onPopDivClick(this);"
							onkeydown="onKeyDownEvent();" onkeyup="onKeyUpEvent();"
							onclick="onPopDivClick(this);"
							autoid=859
							maxlength="20" size="15" name="projestPartner" value="${newCustomer.projestPartner}" />
						</td>
						<td class="data_tb_alignright">
							<c:choose>
								<c:when test="${manager=='no'}">项目负责人</c:when>
								<c:otherwise>高级经理<font color="red" size=3>[*]</font>：</c:otherwise>
							</c:choose>
						</td>
						<td class="data_tb_content">
						<input type="text" id="projestManager"  
							maxlength="20" size="15" name="projestManager"
							<c:if test="${manager!='no'}">
							onfocus="onPopDivClick(this);"
							onkeydown="onKeyDownEvent();" onkeyup="onKeyUpEvent();"
							onclick="onPopDivClick(this);"  
							autoid=858</c:if> 
							value="${newCustomer.projestManager}" /></td>
...
if("${audit}" =="audit"){
	...
	for(var i = 0;i<dmAll.length;i++){
		...
		if(dmAll[i].type =="text"){
			...
			if(dmAll[i].id !="projestPartner" && dmAll[i].id !="projestManager"){
...
//		 var projestManager = document.getElementById("projestManager").value;  //高级经理
		 ...
		 if(projestPartner ==""){
			 alert("必须要选择我所的合伙人才可以通过!");
		 	return ;
		 }
//		 document.thisForm.action="${pageContext.request.contextPath}/newCustomer.do?method=auditPass";
		 document.thisForm.action="${pageContext.request.contextPath}/newCustomer.do?method=auditHeadPass";
