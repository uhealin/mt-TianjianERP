...
Ext.onReady(function(){
	var mytab = new Ext.TabPanel({
        ...
        items:[
            {contentEl: "noApproval",title:"未审批",id:"noApprovalTab"},
            {contentEl: "isApproval",title:"已审批",id:"isApprovalTab",html:
            	"<iframe src=\"${pageContext.request.contextPath}/newCustomer.do?method=headApprovalList\" style=\"width: 100%;height: 550\" id=\"approval\" scrolling=\"no\"></iframe>"}
        ]
    });
...
<div id="divTab">
	<div id="noApproval">
		...
		<div style="height:expression(document.body.clientHeight-55);overflow:auto;">
			<mt:DataGridPrintByBean name="auditNewCustomerList"/>
		</div>
	</div>
</div>
<div id="isApproval" style="overflow: visible;height: 550"></div>
</form>
</div>
<Script>
...
// 删除 
function goDelete(){
	alert(document.getElementById("chooseValue_newCustomerList").value);
	if(document.getElementById("chooseValue_newCustomerList").value==""){
		alert("请选择一项！");
		return ;
	}
...
function goEdit()
{
	if(document.getElementById("chooseValue_auditNewCustomerList").value=="")
	{
		alert("请选择一项！");
		return ;
	}
	...
	{
		window.location="newCustomer.do?method=updateSkip&uuid="+document.getElementById("chooseValue_auditNewCustomerList").value+"&audit=audit";
	}
}
