...
Ext.onReady(function(){
	...
	tabs.on("tabchange",function(tabpanel,tab) {
		if(tab.id == "cur2") {
			goSearch_alreadyDestroyAuditList();			
		}
	}) ;
		//未审批
		
		goSearch_destroyAuditList();
		...
		items:[
			{
				...
				handler:function () {
					var id =  document.getElementById("chooseValue_destroyAuditList").value;
					if(id == "") {
				...
				handler:function () {
					print_destroyAuditList();
				}
			},'->'
        ]
        });  
        //已审批
        var tbars = new Ext.Toolbar({
		renderTo: 'divBtn1',
		items:[
			{
				text:'查询',
...
<div id="my-tab">
	...
	<div id="tab1">
		...
		<div style="height:expression(document.body.clientHeight-58);" >
			<mt:DataGridPrintByBean name="destroyAuditList" />
		</div>
	</div> 
	<div id="tab2">
		<div id="divBtn1"></div> 
		<div style="height:expression(document.body.clientHeight-58);" >
			<mt:DataGridPrintByBean name="alreadyDestroyAuditList" />
		</div>
	</div> 
...
function goAudit(){
	...
	if(id==""){
		...
		window.location = "${pageContext.request.contextPath}/leave.do?method=auditSkip&taskId="+id+"&ctype=destroy"; 
	}
}
