...
<input name="view" type="button" class="flyBT" value=" 审 批  " onclick="audit()">
<!-- 
<input name="view" type="button" class="flyBT" value=" 只显示未审批的申请  " onclick="showNo();">
 -->
<input name="view" type="button" class="flyBT" value=" 打 印 " onclick="print_rightauditList();">
...
<fieldset id="serachFrame">
	...
	<table border="0" cellpadding="0" cellspacing="0" width="100%" bgcolor="">
		...
		<tr>
			...
			<input type="button" name="srh" value="搜 索" onclick="goSearch_rightauditList();">&nbsp;&nbsp;&nbsp;&nbsp;
			<input type="button" name="csrh" value="清 除" onclick="clearF();goSearch_rightauditList();">
			</td>
		</tr>
	</table>
</fieldset>
...
<mt:DataGridPrintByBean name="rightauditList"  outputData="true" outputType="invokeSearch"/>	
<table width="30%" cellspacing="0" cellpadding="0"><tr><td height="10"></td></tr></table>
...
function audit()
{
	window.location="${pageContext.request.contextPath}/role.do?method=auditAdd&&chooseValue="+document.getElementById("chooseValue_rightauditList").value;
}


function submitAndsendMessage(){

	if(document.getElementById("chooseValue_rightaudit").value=="")
	{
		alert("请选择要申请的菜单项！");
	}
	else
	{
		if(confirm("确定删除此角色？")){
			window.location="role.do?method=del&&act=del&&chooseValue="+document.getElementById("chooseValue_rightaudit").value;
		}
	}
