...
function goAdd(){
	document.getElementById("thisForm").action = "${pageContext.request.contextPath}/getFunds.do?method=goDH&opt=add"; 
...
//编辑
function goEdit(){
	var id = document.getElementById("chooseValue_GetFundsList").value;
	if(id==""){
		alert("请选择一项！");
	}else{
		document.getElementById("thisForm").action = "${pageContext.request.contextPath}/getFunds.do?method=goDH&opt=update&id="+id; 
		document.getElementById("thisForm").submit();
	}
}
