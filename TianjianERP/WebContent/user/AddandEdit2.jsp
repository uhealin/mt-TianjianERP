...
<div id="tabUser1" style="padding:10 10 10 10;">
	...
	<tr>
		...
		<td class="data_tb_content" colspan="2" ><input name="name" type="text" id="name"  maxlength="20"  onBlur="query();" size="25" class="required" title="请输入，不得为空"></td>
		<td class="data_tb_content" rowspan="5" >
			<img id="bill" height="120" width="100" src="${pageContext.request.contextPath}${userPhotoSrc}" onerror="this.src='${pageContext.request.contextPath}/images/noPhoto.gif'"><br>
			<input type="text" id="myText" size="1" style="display:none"> <br>
			<a href="javascript:void(0);" onclick="queryWinFun();">&nbsp;点击上传相片</a>
			<c:if test="${userPhotoSrc !='' }">
	...
	<tr>
		<c:choose>
		    ...
		    <td class="data_tb_alignright"  width="20%" align="right">职级：</td>
			<td class="data_tb_content" colspan="2">
				<input name="rank" type="text" id="rank" size="25"   title="请输入，不得为空" onKeyDown="onKeyDownEvent();" onKeyUp="onKeyUpEvent();" noinput="true" onClick="onPopDivClick(this);" valuemustexist=true autoid=864 autoHeight=150>
	     	 </td>
		 </c:when>
	    <c:otherwise>
	    	<td class="data_tb_alignright"  width="20%" align="right">职级<span class="mustSpan">[*]</span>：</td>
			<td class="data_tb_content" colspan="2">
			<input name="rank" type="text" id="rank" size="25"   title="请输入，不得为空" onKeyDown="onKeyDownEvent();" onKeyUp="onKeyUpEvent();" noinput="true" onClick="onPopDivClick(this);" valuemustexist=true autoid=864 autoHeight=150>
			<c:if test="${userSession.userLoginId != 'admin' && !empty(param.myControl) || UserOpt==3}">
			      <script>setObjDisabled("rank");</script>
			</c:if>
		     </td>
	    </c:otherwise>
    	</c:choose>
	...
	<tr>
			...
			<td class="data_tb_content" colspan="2"><input name="roles" id="roles" type="text" id="roles"  size="25"   multiselect="true"  title="请输入，不得为空" onKeyDown="onKeyDownEvent();" onKeyUp="onKeyUpEvent();"  onClick="onPopDivClick(this);" valuemustexist=true autoid=178></td>
		 </c:when>
	    <c:otherwise>
	    	<td class="data_tb_alignright"  width="20%" align="right">操作权限<span class="mustSpan">[*]</span>：</td>
			<td class="data_tb_content" colspan="2" ><input name="roles" id="roles" type="text" id="roles"  size="25"  class="required"  multiselect="true"  title="请输入，不得为空" onKeyDown="onKeyDownEvent();" onKeyUp="onKeyUpEvent();"  onClick="onPopDivClick(this);" valuemustexist=true autoid=178></td>
	    </c:otherwise>
    	</c:choose>
	</tr>
	<tr>
		<td class="data_tb_alignright"  width="20%" align="right">所属单位<span class="mustSpan">[*]</span>：</td>
		<td class="data_tb_content" colspan="2"><input name="departid" type="text" id="departid"  onKeyDown="onKeyDownEvent();" onKeyUp="onKeyUpEvent();" onClick="onPopDivClick(this);" autoid="4" size=25  maxlength="50"></td>
	</tr>
...
<c:if test="${temp==1}">	
	...
	<tr>
		    ...
		    <c:when test="${userSession.userLoginId=='admin'}"> 
				...
				<input name="departmentid" type="hidden" id="departmentid" size="25"  >
				<input name="departmentName" type="text" id="departmentName" size="25"  title="请输入，不得为空" onKeyDown="onKeyDownEvent();" onKeyUp="onKeyUpEvent();" noinput="true" onClick="onPopDivClick(this);" valuemustexist=true autoid=958 multilevel=true autoHeight=150>
				<c:if test="${UserOpt!=3}">
		    ...
		    <c:otherwise>
		    	<td class="data_tb_alignright"  width="20%" align="right">所属部门<span class="mySpan">[*]</span>：</td>
				<td class="data_tb_content">
				<input name="departmentid" type="hidden" id="departmentid" size="25"  >
				<input name="departmentName" type="text" id="departmentName" size="25" class="required"  title="请输入，不得为空" onKeyDown="onKeyDownEvent();" onKeyUp="onKeyUpEvent();" noinput="true" onClick="onPopDivClick(this);" valuemustexist=true autoid=715 autoHeight=150>
		    </c:otherwise>
    		</c:choose>
	     	<c:if test="${userSession.userLoginId != 'admin' && !empty(param.myControl) || UserOpt==3}">
			      <script>setObjDisabled("departmentName");</script>
			</c:if>
		<c:choose>
		    <c:when test="${userSession.userLoginId=='admin'}"> 
			<td class="data_tb_alignright" align="right" width="20%">身份证号：</td>
			<td class="data_tb_content">
				<input name="identityCard" type="text" id="identityCard" size=25  maxlength="18">
			</td>
		 </c:when>
	    <c:otherwise>
	    	<td class="data_tb_alignright" align="right" width="20%">身份证号<span class="mySpan">[*]</span>：</td>
			<td class="data_tb_content">
				<input name="identityCard" type="text" id="identityCard" size=25  maxlength="18" class="required">
			</td>
	    </c:otherwise>
    	</c:choose>
	</tr>	
	<tr>
		<td class="data_tb_alignright"  width="20%" align="right">楼层：</td>
		<td class="data_tb_content"><input title='用户所在楼层不能为空'   name="floor" maxlength="50" type="text" id="floor"  size="25"  ></td>
		<!--  
		<td class="data_tb_alignright"  width="20%" align="right">房间号：</td>
		<td class="data_tb_content"><input name="house" maxlength="50" type="text" id="house"  size="25"  title="请填写您所在的房间号"></td>
	    -->
	</tr>
	<tr>
		<td class="data_tb_alignright"  width="20%" align="right">工号：</td>
		<td class="data_tb_content"><input name="station" maxlength="50" type="text" id="station"  size="25"  title="请填写您的工位号"></td>
		
		
		<c:choose>
		    <c:when test="${userSession.userLoginId=='admin'}"> 
			<td class="data_tb_alignright"  width="20%" align="right">手机：</td>
			<td class="data_tb_content"><input class="validate-digits" name="mobilePhone" maxlength="11" type="text" id="mobilePhone"  size="25"  title="手机号码不能为空"></td>
		 </c:when>
	    <c:otherwise>
	    	<td class="data_tb_alignright"  width="20%" align="right">手机<span class="mySpan">[*]</span>：</td>
		<td class="data_tb_content"><input class="required validate-digits" name="mobilePhone" maxlength="11" type="text" id="mobilePhone"  size="25"  title="手机号码不能为空"></td>
	    </c:otherwise>
    	</c:choose>
	...
	<tr>
		<c:choose>
		    <c:when test="${userSession.userLoginId=='admin'}"> 
				...
				<td class="data_tb_content" ><input class="phonenumber-wheninputed" title='请输入正确的办公电话' name="phone"  maxlength="13" type="text" id="phone"  size="25"  ></td>
			 </c:when>
	    <c:otherwise>
	    		<td class="data_tb_alignright"  width="20%" align="right">办公电话<span class="mySpan">[*]</span>：</td>
				<td class="data_tb_content" ><input class="required phonenumber-wheninputed" title='请输入正确的办公电话' name="phone"  maxlength="13" type="text" id="phone"  size="25"  ></td>
	    </c:otherwise>
    	</c:choose>
    	
		<td class="data_tb_alignright"  width="20%" align="right">邮箱：</td>
		<td class="data_tb_content" ><input name="email" maxlength="50" type="text" class="validate-email" id="email"  size="25"  title="请正确填写您的邮箱"></td>
	</tr>	
	</table>
...
<div id="tabUser2" class="x-hide-display" style="padding:10 10 10 10;">
	...
	<tr>
		...
		<td class="data_tb_content" >
			<input id="emtype" name="emtype" value="${uservo.emtype}" autoid="700" refer="人员库">
		</td>
	</tr>
	...
	<input type="hidden"   value="${close }" name="close" id="close">	
	</fieldset>
	</div>
...
<!-- Y B -->
<script language="javascript" >
// 不要注释，要不然 修改时将不显示 所属部门
document.getElementById("departmentName").value = document.getElementById("departmentid").value;
	...
	Ext.Ajax.request({
		...
		success:function (response,options) {
			if(response.responseText != "ok") {
				...
				document.thisForm.action="user.do?method=Save&close=${close}&pd="+pd;
...
Ext.onReady(function(){
	...
	setObjDisabled("roles");
	setObjDisabled("rank");
	setObjDisabled("departmentName");
	setObjDisabled("floor"); 
	setObjDisabled("station");
	setObjDisabled("house");
	setObjDisabled("identityCard"); 
	setObjDisabled("email");
});
</script>
</c:if>
