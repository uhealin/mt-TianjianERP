...
<% 
   ...
   try{
	   ...
	   re=dbUtil.queryForInt("select count(*) from k_userrole where userid="+userSession.getUserId()+" and rid=236");	   
	...
	var tbar_customer = new Ext.Toolbar({
        ...
        <% }%>
        {
			...
			cls:'x-btn-text-icon',
	   		icon:'${pageContext.request.contextPath}/img/query.gif',
			handler:function(){
				var ids = getChooseValue("newsList_${param.menuid}");
				window.location="news.do?method=viewNews&&autoId="+ids;
			}
		},'-',{
	            text:'关闭',
	            cls:'x-btn-text-icon',
...
<mt:DataGridPrintByBean name="newsList_${param.menuid}"/>
</div>
<Script>
...
function goAdd()
{
	window.location="news.do?method=goAdd&opt=${param.opt}&menuid=${param.menuid}&hasclass=${param.hasclass}";
}
...
function goDelete(){
	...
	if(ids == "" ) {
		...
		if(confirm("删除此项将导致资料丢失，是否继续")){
			window.location="news.do?method=deleteNews&&autoId="+ids+"&opt=<%=request.getParameter("opt")%>&menuid=${param.menuid}&hasclass=${param.hasclass}";
		}	
	}
...
function goEdit()
{
	...
	if(ids == "" ) {
		...
		window.location="news.do?method=goUpdate&&autoId="+ids+"&opt=<%=request.getParameter("opt")%>&hasclass=${param.hasclass}";
	}
}
