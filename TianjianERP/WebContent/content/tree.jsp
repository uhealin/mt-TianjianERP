...
function tree(divName){
	...
	var data=new Ext.tree.TreeLoader({
		url:'${pageContext.request.contextPath}/content.do?method=tree'
	}); 
	...
	*/
	tree.on('click',function(node,event){
		...
		//类型：本人|所有
 		var url = "${pageContext.request.contextPath}/content.do?op=${param.op}&rand="+Math.random();
 		if(node.attributes.classid != undefined ){
 			url +="&classid=" +node.attributes.classid; 
 		}
...
<div id="centerDiv" class="x-hide-display">
			<div style="height:expression(document.body.clientHeight+3);" >
				<iframe id="divIframe" name="divIframe" scrolling="auto" frameborder="0" width="100%" height="100%" src="${pageContext.request.contextPath}/content.do?op=${param.op}&classid=ID001" ></iframe>
			</div>
 </div>
